...
export default function ProfileSettings({ oneClickProfile }) {
    ...
    return (
        <AppLayout>
            <Head title={`${t('accountSettings.title')} | Takeer`} />
            <div className="max-w-5xl mx-auto p-4 md:p-8 space-y-6 pb-24">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-black tracking-tight text-foreground">{t('accountSettings.yourSettings')}</h1>
                        <p className="text-sm text-muted-foreground">{t('accountSettings.settingsDescription')}</p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    ...
                    <Card className="border-border shadow-sm">
                        <CardContent className="p-6">
                            <div className="flex items-center gap-2 mb-4 text-brand-700">
                                ...
                                <h2 className="font-bold text-lg text-foreground">{t('accountSettings.email')}</h2>
                            </div>
                            ...
                            {auth?.user?.email ? (
                                <div className="flex items-center justify-between p-4 bg-muted/30 border border-input rounded-xl">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-green-600 font-medium flex items-center gap-1"><CheckCircle2 className="h-3 w-3" /> {t('accountSettings.connected')}</p>
                                        </div>
                                    </div>
                                    <a href="/auth/google/redirect" className="text-sm font-semibold text-brand-600 hover:text-brand-700 ml-2">{t('accountSettings.change')}</a>
                                </div>
                            ) : (
                                <a
                                ...
                                >
                                    <svg className="w-5 h-5" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" /><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" /><path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" /><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" /><path fill="none" d="M1 1h22v22H1z" /></svg>
                                    {t('accountSettings.connectGoogle')}
                                </a>
                            )}
                        </CardContent>
                    </Card>
                        ...
                        <CardContent className="p-6">
                            <div className="flex items-center justify-between mb-4">
                                <div className="flex items-center gap-2 text-brand-700">
                                    ...
                                    <h2 className="font-bold text-lg text-foreground">{t('accountSettings.paymentMethod')}</h2>
                                </div>
                                <ShieldCheck className="h-5 w-5 text-green-500" />
                            </div>
                            <p className="text-sm text-muted-foreground mb-4">
                                {t('accountSettings.paymentDescription')}
                            </p>

                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <label className="text-sm font-semibold">{t('accountSettings.choosePayment')}</label>
                                    <select
                                    ...
                                    >
                                        <option value="mobile_money">{copy('Mobile Money (M-Pesa, Tigo, Airtel, MTN, etc.)', 'Mobile Money (M-Pesa, Tigo, Airtel, MTN, n.k)')}</option>
                                        <option value="card" disabled>{copy('Bank card (Coming soon)', 'Kadi ya Benki (Hivi Karibuni)')}</option>
                                    </select>
                                    {errors.payment_provider && <p className="text-sm text-red-500 font-medium">{errors.payment_provider}</p>}
                                </div>

                                <div className="space-y-2">
                                    <label className="text-sm font-semibold">{t('accountSettings.paymentNumber')}</label>
                                    <Input
                                        type="tel"
                                        placeholder={copy('E.g. +255755000000', 'Mf. +255755000000')}
                    ...
                    <Card className="border-border shadow-sm">
                        <CardContent className="p-6">
                            <div className="flex items-center gap-2 mb-4 text-brand-700">
                                ...
                                <h2 className="font-bold text-lg text-foreground">{t('accountSettings.deliveryAddress')}</h2>
                            </div>
                            <p className="text-sm text-muted-foreground mb-6">
                                {t('accountSettings.addressDescription')}
                            </p>
                    ...
                    >
                        {processing ? t('accountSettings.saving') : t('accountSettings.saveSettings')}
                    </Button>
