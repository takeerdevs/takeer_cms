    ...
    return (
        <div className="space-y-3 rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            <div className="flex items-start justify-between gap-3">
                <div>
                    <p className="text-xs font-black uppercase tracking-wider text-slate-700">{copy('Product FAQ', 'Maswali ya bidhaa')}</p>
                    <p className="text-xs text-slate-500">{copy('Questions and answers shown on the product page.', 'Maswali na majibu yatakayoonekana kwenye ukurasa wa bidhaa.')}</p>
                </div>
                <button
                    type="button"
                ...
                >
                    {copy('Add FAQ', 'Ongeza swali')}</button>
                ...
                {productFaqs.map((faq, index) => (
                    <div key={`product-faq-${index}`} className="space-y-2 rounded-xl border border-slate-200 bg-white p-3">
                        ...
                        <Input
                            ...
                            placeholder={copy('E.g. Does it come with charger?', 'Mf. Je, inakuja na chaja?')}
                        />
                        <Textarea
                            ...
                            placeholder={copy('E.g. Yes, it includes 1 charging cable and manual.', 'Mf. Ndiyo, inakuja na cable moja ya kuchaji na mwongozo.')}
                        />
