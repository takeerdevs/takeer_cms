...
export default function DigitalDownloadModal({ isOpen, onClose, orderId, entitlementId, productTitle, productId, accessProduct = null }) {
    ...
    const hydrateAccessProduct = (product) => {
        ...
        setDownloadMessage(t('components.membershipInOrders'));
    ...
    const fetchDownloadUrl = async () => {
        ...
        try {
            ...
            toast.error(e.response?.data?.message || t('publicCommerce.contentOpenFailed'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDownload = () => {
        ...
        setDownloaded(true);
        toast.success(t('components.downloadFile'));
    };
    ...
    const localizedMetaLabel = {
        pdf: t('components.pdfDocument'), mp4: t('components.videoFile'), mp3: t('components.audioFile'), zip: t('components.archiveFile'), rar: t('components.archiveFile'), jpg: t('components.imageFile'), jpeg: t('components.imageFile'), png: t('components.imageFile'),
    }[meta.ext] || t('components.digitalFile');
            ...
            >
                ...
                <div className={`bg-gradient-to-br ${isCourse || isStream ? 'from-indigo-600 to-indigo-900' : isGallery ? 'from-blue-600 to-cyan-800' : meta.color} px-6 pt-8 pb-12 text-white relative overflow-hidden`}>
                    ...
                    <div className="relative z-10 flex flex-col items-center text-center gap-3">
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/70">{t('components.paymentComplete')}</p>
                            <h2 className="text-2xl font-black tracking-tight mt-1 leading-tight text-white">{productTitle || t('components.yourProduct')}</h2>
                            <p className="text-sm font-bold text-white/90 mt-1">{isStream ? (isAudioStream ? t('components.listenAudio') : t('components.watchVideo')) : isCourse ? t('publicCommerce.courseBundle') : isGallery ? t('components.openGallery') : localizedMetaLabel}{meta.ext && !isStream && !isGallery ? ` (.${meta.ext})` : ''}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="-mt-8 mx-5 rounded-[2rem] bg-white border border-slate-100 shadow-xl p-6 space-y-5 relative z-10">
                    ...
                    {(fileSize || meta.ext || isCourse || isStream || isGallery) && (
                            ...
                            <div>
                                <p className={`text-xs font-black uppercase tracking-widest ${isCourse || isStream ? 'text-indigo-700' : isGallery ? 'text-blue-700' : meta.text}`}>{isStream ? t('components.protectedStreaming') : isCourse ? t('components.structuredLearning') : isGallery ? t('components.unlockedGallery') : localizedMetaLabel}</p>
                                {isCourse || isStream || isGallery ? (
                                    <p className="text-sm font-bold text-slate-700 mt-0.5">{isStream ? (isAudioStream ? t('components.listenInside') : t('components.watchInside')) : isGallery ? t('components.galleryInside') : t('components.lessonsReady')}</p>
                                ) : fileSize && (
                    ...
                    {loading ? (
                        <div className="flex flex-col items-center gap-3 py-6">
                            ...
                            <p className="text-sm font-bold text-slate-500">{t('components.preparingFile')}</p>
                        </div>
                    ) : fileUrl ? (
                        isStream && !isAudioStream ? (
                            ...
                            >
                                {isCourse || isStream ? (
                                    <><PlayCircle className="h-5 w-5" /> {isStream ? (isAudioStream ? t('components.listenAudio') : t('components.watchVideo')) : t('components.openLessons')}</>
                                ) : isGallery ? (
                                    <><Image className="h-5 w-5" /> {t('components.openGallery')}</>
                                ) : downloaded ? (
                                    <><CheckCircle2 className="h-5 w-5" /> {t('components.downloadAgain')}</>
                                ) : (
                                    <><Download className="h-5 w-5" /> {t('components.downloadFile')}</>
                                )}
                            </button>
                        )
                    ) : isCustomPending ? (
                        <div className="rounded-2xl border border-amber-100 bg-amber-50 p-4 text-center">
                            ...
                            <p className="mt-2 text-sm font-black text-amber-900">{t('components.customPreparing')}</p>
                            <p className="mt-1 text-xs font-semibold leading-5 text-amber-800">
                                {downloadMessage || t('components.merchantPreparing')}
                            </p>
                        </div>
                    ) : (
                        <button
                        ...
                        >
                            <Download className="h-5 w-5" /> {t('components.retry')}
                        </button>
                    )}
                    ...
                    {softwareLicenseKey?.key && (
                        <div className="rounded-2xl border border-emerald-100 bg-emerald-50 p-4">
                            <p className="text-[10px] font-black uppercase tracking-wider text-emerald-700">{t('components.licenseKey')}</p>
                    ...
                    <p className="text-[10px] text-center leading-relaxed">
                        {orderId ? t('components.fileInOrders') : t('components.membershipInOrders')}
                    </p>
                </div>
