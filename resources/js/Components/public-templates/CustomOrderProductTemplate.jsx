        ...
        <AppLayout hideTabBar>
            ...
            <div className="min-h-screen bg-[#fbfaf7] pb-28 text-slate-950">
                    ...
                    <div className="relative z-10 mx-auto flex min-h-[440px] max-w-5xl flex-col justify-between p-4 md:p-8">
                        ...
                        <div className="max-w-4xl">
                            ...
                            <div className="mt-4 flex flex-wrap gap-4 text-sm font-bold text-white/90">
                                ...
                                {minimumOrder > 1 && <span className="inline-flex items-center gap-1.5"><Package className="h-4 w-4" />{t('template.minimum')} {minimumOrder}</span>}
                ...
                <main className="mx-auto grid max-w-5xl gap-5 p-4 md:grid-cols-[minmax(0,1fr)_380px] md:p-8">
                    <div className="space-y-5">
                        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                            <h2 className="text-lg font-black">{t('template.aboutCustomOrder')}</h2>
                            <p className="mt-3 text-sm leading-7 text-slate-700">{product.description || product.attributes?.suggested_description || t('template.customOrderFallback')}</p>
                            <div className="mt-5 grid gap-3 sm:grid-cols-3">
                                <Stat icon={ClipboardList} label={t('template.quoteFlow')} value={quotePolicy} />
                                <Stat icon={Clock3} label={t('template.leadTime')} value={details.lead_time || t('template.afterRequest')} />
                                <Stat icon={Package} label={t('template.minimum')} value={minimumOrder > 1 ? minimumOrder : t('template.none')} />
                            </div>
                        </section>

                        {details.customization_notes && (
                            <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                                <h2 className="text-lg font-black">{t('template.whatToSpecify')}</h2>
                        ...
                        {details.pickup_delivery_notes && (
                            <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h2 className="text-lg font-black">{t('template.pickupDelivery')}</h2>
                                </div>
                        ...
                        <section className="rounded-2xl bg-purple-50 p-5 text-purple-950 ring-1 ring-purple-100">
                            <h2 className="text-lg font-black">{t('template.howRequestWorks')}</h2>
                            <div className="mt-4 space-y-3">
                                {[t('template.howStepQuantity'), t('template.howStepMerchant'), t('template.howStepPayment')].map((step, index) => (
                        ...
                        {merchantSlug && (
                            <Link href={`/m/${merchantSlug}`} className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5 transition hover:bg-brand-50">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <p className="font-black">{merchant?.display_name || t('template.customProvider')}</p>
                                        <p className="text-sm text-slate-500">{t('template.viewMoreBusiness')}</p>
                                    </div>
                                </div>
                    ...
                    <aside className="space-y-4 md:sticky md:top-5 md:self-start">
                        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                            <p className="text-xs font-black uppercase tracking-wide text-slate-500">{unitPrice > 0 ? t('template.startingPrice') : t('template.quoteRequest')}</p>
                            <p className="mt-1 text-3xl font-black text-brand-700">{unitPrice > 0 ? `TZS ${unitPrice.toLocaleString()}` : t('template.confirmQuote')}</p>
                            <div className="mt-5 flex items-center justify-between rounded-xl bg-slate-50 p-2">
                                ...
                                <div className="text-center">
                                    ...
                                    <p className="text-[11px] font-bold uppercase text-slate-500">{t('template.quantity')}</p>
                                </div>
                            ...
                            {minimumOrder > 1 && <p className="mt-2 text-xs font-semibold text-slate-500">{t('template.minimumOrder', { count: minimumOrder })}</p>}
                        ...
                        <section className="rounded-2xl bg-emerald-50 p-4 text-emerald-950 ring-1 ring-emerald-100">
                            <div className="flex gap-3">
                                ...
                                <div>
                                    <p className="font-black">{t('template.customProtected')}</p>
                                    <p className="mt-1 text-sm text-emerald-900/75">{t('template.confirmCustom')}</p>
                                </div>
                            </div>
                        </section>
                    </aside>
                ...
                <ServiceRequestModal
                    ...
                    title={t('template.customRequest')}
                    submitLabel={t('template.sendQuoteRequest')}
                    modulePayload={{
