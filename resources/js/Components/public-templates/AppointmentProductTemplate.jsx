                    ...
                    <div className="relative z-10 mx-auto flex min-h-[420px] max-w-5xl flex-col justify-between p-4 md:p-8">
                        ...
                        <div className="max-w-4xl">
                            ...
                            <div className="mt-4 flex flex-wrap gap-4 text-sm font-bold text-white/90">
                                <span className="inline-flex items-center gap-1.5"><Clock3 className="h-4 w-4" />{details.appointment_duration_minutes || 60} {t('template.minute')}</span>
                                <span className="inline-flex items-center gap-1.5"><Users className="h-4 w-4" />{capacity} {t('template.capacity')}</span>
                ...
                <main className="mx-auto grid max-w-5xl gap-5 p-4 md:grid-cols-[minmax(0,1fr)_380px] md:p-8">
                    <div className="space-y-5">
                        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                            <h2 className="text-lg font-black">{t('template.aboutAppointment')}</h2>
                            <p className="mt-3 text-sm leading-7 text-slate-700">{product.description || product.attributes?.suggested_description || t('template.appointmentFallback')}</p>
                            <div className="mt-5 grid gap-3 sm:grid-cols-3">
                                <Stat icon={Clock3} label={t('template.duration')} value={`${details.appointment_duration_minutes || 60} min`} />
                                <Stat icon={Sparkles} label={t('template.buffer')} value={`${details.buffer_minutes ?? 15} min`} />
                                <Stat icon={Users} label={t('template.capacity')} value={capacity} />
                            </div>
                        </section>

                        {details.preparation_notes && (
                            <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                                <h2 className="text-lg font-black">{t('template.beforeArrive')}</h2>
                                <p className="mt-3 whitespace-pre-line text-sm leading-7 text-slate-700">{details.preparation_notes}</p>
                            </section>
                        )}

                        {merchantSlug && <MerchantLink merchant={merchant} merchantSlug={merchantSlug} label={t('template.viewProvider')} />}
                    </div>
                    ...
                    <CheckoutPanel
                        ...
                        total={total}
                        count={spots}
                        countLabel={spots === 1 ? t('template.spot') : t('template.spots')}
                        max={capacity}
                        canSubmit={canBook}
                        submitLabel={canBook ? t('template.requestAppointment') : t('template.full')}
                        onMinus={() => setSpots((value) => Math.max(1, value - 1))}
                        onPlus={() => setSpots((value) => Math.min(capacity || 100, value + 1))}
                        onSubmit={openBooking}
                    />
                </main>
                <MobileBar total={total} label={t('template.book')} disabled={!canBook} onSubmit={openBooking} />
                <ServiceRequestModal
                    product={product}
                    open={requestOpen}
                    onOpenChange={setRequestOpen}
                    requestType="appointment_request"
                    title={t('template.requestAppointment')}
    ...
    return (
        <aside className="space-y-4 md:sticky md:top-5 md:self-start">
            <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                ...
                {max > 0 && <p className="mt-2 text-xs font-semibold text-slate-500">{t('template.maximumBooking', { max })}</p>}
            ...
            <section className="rounded-2xl bg-sky-50 p-4 text-sky-950 ring-1 ring-sky-100">
                <div className="flex gap-3">
                    ...
                    <div>
                        <p className="font-black">{t('template.protected')}</p>
                        <p className="mt-1 text-sm text-sky-900/75">{t('template.confirmAppointment')}</p>
                    </div>
                </div>
            </section>
        </aside>
