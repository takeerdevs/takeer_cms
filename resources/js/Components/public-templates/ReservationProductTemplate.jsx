        ...
        <AppLayout hideTabBar>
            ...
            <div className="min-h-screen bg-[#fbfaf7] pb-28 text-slate-950">
                ...
                <main className="mx-auto grid max-w-5xl gap-5 p-4 md:grid-cols-[minmax(0,1fr)_380px] md:p-8">
                    <div className="space-y-5">
                        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                            <h2 className="text-lg font-black">{t('template.reservationDetails')}</h2>
                            <p className="mt-3 text-sm leading-7 text-slate-700">{product.description || product.attributes?.suggested_description || t('template.confirmReservation')}</p>
                            <div className="mt-5 grid gap-3 sm:grid-cols-3">
                                <Stat icon={Sofa} label={t('template.type')} value={reservationType} />
                                <Stat icon={Users} label={t('template.capacity')} value={partyLimit > 0 ? t('template.guestsUpTo', { count: partyLimit }) : t('template.open')} />
                                <Stat icon={Clock3} label={t('template.duration')} value={`${details.reservation_duration_minutes || 90} min`} />
                            </div>
                        </section>

                        {(depositAmount > 0 || details.deposit_note || details.reservation_notes) && (
                            <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                                <h2 className="text-lg font-black">{t('template.policyNotes')}</h2>
                                {depositAmount > 0 && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-sm font-semibold leading-6 text-amber-950">{t('templateExtra.deposit')}: TZS {depositAmount.toLocaleString()}</p>}
                    ...
                    <aside className="space-y-4 md:sticky md:top-5 md:self-start">
                        <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-black/5">
                            <p className="text-xs font-black uppercase tracking-wide text-slate-500">{price > 0 ? t('template.perGuest') : t('template.reservation')}</p>
                            <p className="mt-1 text-3xl font-black text-brand-700">{price > 0 ? `TZS ${Number(price).toLocaleString()}` : t('template.confirmQuote')}</p>
                            ...
                            <Button className="mt-4 h-12 w-full rounded-xl text-base font-black" disabled={!canReserve} onClick={openBooking}>
                                ...
                                {canReserve ? t('template.requestBooking') : t('template.full')}
                            </Button>
                        </section>
                        <TrustNote />
                    </aside>
                ...
                <ServiceRequestModal
                    ...
                    title={t('template.requestBooking')}
    ...
    return (
        <Link href={`/m/${merchantSlug}`} className="flex items-center justify-between rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/5 transition hover:bg-brand-50">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <p className="font-black">{merchant?.display_name || t('merchantUi.businessSettings')}</p>
                    <p className="text-sm text-slate-500">{t('template.viewMoreReservations')}</p>
                </div>
            </div>
    ...
    return (
        <section className="rounded-2xl bg-orange-50 p-4 text-orange-950 ring-1 ring-orange-100">
            <div className="flex gap-3">
                ...
                <div>
            <p className="font-black">{t('template.reservationProtected')}</p>
            <p className="mt-1 text-sm text-orange-900/75">{t('template.confirmReservation')}</p>
                </div>
            </div>
        </section>
    );
