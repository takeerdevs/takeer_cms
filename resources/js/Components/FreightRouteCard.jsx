...
export const freightModeLabel = (mode, copy = (english) => english) => copy(
    freightModeLabels[mode] || String(mode || '').replace(/_/g, ' '),
    {
        sea_cargo: 'Cargo ya baharini',
        air_cargo: 'Cargo ya anga',
    }[mode] || String(mode || '').replace(/_/g, ' '),
);

export const freightPriceLabel = (detail = {}, copy = (english) => english) => {
    if (!detail?.price_amount) return detail?.pricing_model === 'quote' ? copy('Quote', 'Quote') : '';
    const suffix = detail.pricing_model && detail.pricing_model !== 'quote'
        ? ` ${copy(freightPricingSuffix[detail.pricing_model] || detail.pricing_model, freightPricingSuffix[detail.pricing_model] || detail.pricing_model)}`
        : '';
    return `${detail.currency || ''} ${detail.price_amount}${suffix}`.trim();
};

export const freightEstimateLabel = (estimate, copy = (english) => english) => {
    const value = String(estimate || '').trim();
    if (!value) return '';
    if (/[a-zA-Z]/.test(value)) return value;
    if (/^\d+(\.\d+)?(\s*-\s*\d+(\.\d+)?)?$/.test(value)) {
        return `${value} ${copy(value === '1' ? 'day' : 'days', 'siku')}`;
    }
    return value;
};
...
export default function FreightRouteCard({ snapshot = {}, onOpen, routeHref = null, className = '' }) {
    ...
    const [originFallback = copy('Origin', 'Mwanzo'), destinationFallback = copy('Destination', 'Mwisho')] = String(snapshot.label || '').split(' to ');
    const origin = compactFreightPlace(originLocations, originFallback || copy('Origin', 'Mwanzo'));
    const destination = compactFreightPlace(destinationLocations, destinationFallback || copy('Destination', 'Mwisho'));
                            ...
                            <div key={mode} className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        ...
                                        {detail.estimate && <p className="mt-1 text-xs font-bold text-slate-500">{freightEstimateLabel(detail.estimate, copy)}</p>}
                                    </div>
                ...
                <div className="flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
                    ...
                    {(routeHref || onOpen) ? (
                        ...
                        <span className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-full bg-slate-100 px-4 text-sm font-black text-slate-700">
                            {copy('Route details', 'Maelezo ya route')}
                            <Truck className="h-4 w-4" />
                        </span>
                    )}
                </div>
