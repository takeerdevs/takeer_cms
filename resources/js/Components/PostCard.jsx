...
import { MessageCircle, MoreHorizontal, ShoppingBag, Clock, BadgeCheck, Crown, Unlock, Lock, X, Boxes, Loader2, ShieldCheck, AlertTriangle, Trash2, Eye, MessageSquare, SmilePlus, Utensils, Flame, BedDouble, Users, MapPin, Route, Presentation, CalendarClock, ConciergeBell, Truck, PenLine, Package, PlayCircle, Ship } from 'lucide-react';
...
const compactFreightPlace = (locations = [], fallback = '') => {
...
function FreightRoutePostCard({ snapshot = {}, onOpen }) {
    ...
    return (
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm" onClick={(event) => event.stopPropagation()}>
...
export default function PostCard({ post, readOnly = false, detailHref = null, adminMode = false }) {
    ...
    const forwarderRouteHref = (forwarderRouteSnapshot?.id || postData.forwarder_route_id)
        ? `/freight/routes/${forwarderRouteSnapshot?.id || postData.forwarder_route_id}`
        : null;
    ...
    const shouldShowOpenCta = !isLocked && isLongForm && !attachedProduct && !isBundlePromotable && !isSubscriptionPromotable && !isOfferingGroupPromotable && !isForwarderRoutePost;
    ...
    return (
        <article className={`relative bg-card border-b overflow-hidden ${adminMode && isDeleted ? 'border-rose-300 bg-rose-50/35' : 'border-border'}`}>
            ...
            {isOfferingGroupPromotable && (
                <div className="my-1 overflow-hidden rounded-2xl border border-orange-200/80 bg-[#fffaf2] shadow-sm">
                    <Link
                        href={`/offerings/${offeringGroupRouteKey}`}
                        onClick={(e) => {
                            e.stopPropagation();
                            trackPostCardClick('offering_group', promotableItem);
                        }}
                        className="block"
                    >
                        <div className="relative min-h-40 overflow-hidden bg-slate-950 text-white">
                            ...
                            <div className="relative p-4">
                                ...
                                <h3 className="mt-10 text-3xl font-black leading-none tracking-tight">{promotableItem?.title || postData.title}</h3>
                                {promotableItem?.description && (
                                    <p className="mt-2 line-clamp-2 max-w-xl text-sm font-semibold leading-6 text-white/80">{promotableItem.description}</p>
                                )}
                            </div>
                        </div>
                        ...
                        {offeringGroupTemplate === 'menu_board' ? (
                            <div className="bg-white px-4 py-4">
                                ...
                                <div className="space-y-3">
                                    {offeringGroupItems.slice(0, 5).map((item) => {
                                        const thumbnailUrl = item.thumbnail_url || item.image_url;
                                        const isVideoItem = String(item.media_type || '').toLowerCase() === 'video';

                                        return (
                                            <div key={item.id} className="grid grid-cols-[3.5rem_minmax(0,1fr)_auto] items-center gap-3">
                                                <div className="relative h-14 w-14 overflow-hidden rounded-xl bg-orange-50 ring-1 ring-orange-100">
                                                    {thumbnailUrl ? (
                            ...
                            <div className="grid gap-2 bg-white p-3">
                                {offeringGroupItems.slice(0, 4).map((item, index) => (
                                    <div key={item.id} className="flex items-center gap-3 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                    ...
                    <div className="border-t border-orange-100 bg-white px-4 pb-4 pt-3">
                        <button
                            onClick={(e) => {
                                e.preventDefault();
                                e.stopPropagation();
