...
return new class extends Migration
{
    public function up(): void
    {
        DB::table('countries')
            ->orderBy('id')
            ->select(['id', 'iso_alpha2', 'settings'])
            ->get()
            ->each(function ($country) {
                $settings = $this->decodeSettings($country->settings);
                $defaults = strtoupper((string) $country->iso_alpha2) === 'TZ'
    ...
    public function down(): void
    {
        DB::table('countries')
            ...
            ->each(function ($country) {
                ...
                if (! isset($settings['tax_calendar_defaults'])) {
                    return;
                }

                $generatedKeys = collect($this->baseDefaults())
                    ->merge($this->tanzaniaDefaults())
                    ->pluck('key')
                    ->all();

                $remaining = collect($settings['tax_calendar_defaults'])
                    ->reject(fn ($item) => in_array($item['key'] ?? null, $generatedKeys, true))
                    ->values()
                    ->all();

                if (count($remaining) > 0) {
                    $settings['tax_calendar_defaults'] = $remaining;
                } else {
